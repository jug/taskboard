...
module ApplicationHelper
  ...
  def make_time(time_in_string)
    make_time_from_str(time_in_string).to_i
  end

  def make_time_from_str(time_in_string)
    time = time_in_string.split("-")
    Time.mktime(time[0], time[1], time[2], 0, 0, 0, 0)
  end

  def make_time_from_date(date)
    Time.mktime( date.year, date.month, date.mday, 0, 0, 0, 0)
  end
